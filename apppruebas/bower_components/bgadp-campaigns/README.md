...
CELLS *auto-generated* **data provider** for [BBVA GLOBAL API](https://bbva-apicatalog.appspot.com/#/global/apis/products/campaigns).
...
```bash
$ bower install --save bgadp-campaigns
```
...
```javascript
let dataProvider = BGADPCampaigns.campaigns.campaign.get(config);
