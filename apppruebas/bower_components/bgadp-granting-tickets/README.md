...
```bash
$ bower install --save bgadp-granting-tickets
```

Then add it to your component HTML file:

```html
<link rel="import" href="../bgadp-granting-tickets/bgadp-granting-tickets.html"/>
```

## Sample of use

```javascript
let dataProvider = BGADPGrantingTickets.grantingTickets.grantingTicket.get(config);
