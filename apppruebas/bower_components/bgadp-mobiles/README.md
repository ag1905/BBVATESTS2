...
CELLS *auto-generated* **data provider** for [BBVA GLOBAL API](https://bbva-apicatalog.appspot.com/#/global/apis/mobile/mobiles).
...
```bash
$ bower install --save bgadp-mobiles
```
...
```javascript
let dataProvider = BGADPMobiles.mobiles.mobile.get(config);
