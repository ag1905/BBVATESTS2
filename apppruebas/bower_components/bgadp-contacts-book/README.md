...
CELLS *auto-generated* **data provider** for [BBVA GLOBAL API](https://bbva-apicatalog.appspot.com/#/global/apis/people/contactsBook).
...
```bash
$ bower install --save bgadp-contacts-book
```
...
```javascript
let dataProvider = BGADPContactsBook.contactsBook.contactsBook.get(config);
