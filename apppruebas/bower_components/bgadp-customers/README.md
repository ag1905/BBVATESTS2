...
CELLS *auto-generated* **data provider** for [BBVA GLOBAL API](https://bbva-apicatalog.appspot.com/#/global/apis/people/customers).
...
```bash
$ bower install --save bgadp-customers
```
...
```javascript
let dataProvider = BGADPCustomers.customers.customer.get(config);
