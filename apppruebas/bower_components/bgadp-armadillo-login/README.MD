...
```bash
$ bower install --save bgadp-armadillo-login
```

Then add it to your component HTML file:

```html
<link rel="import" href="../bgadp-armadillo-login/bgadp-armadillo-login.html"/>
```

## Sample of use

```javascript
let dataProvider = BGADPArmadilloLogin.armadilloLogin.armadilloLogin.get(config);
